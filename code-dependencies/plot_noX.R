    ...
    if(!add && !onechr) {
      if(!alternate.chrid || length(xtick) < 2) {
        for(i in seq(along=xtick))
          axis(side=1, at=xtick[i], labels=xticklabel[i], cex.axis = 1.5)
      }
      else {
        odd <- seq(1, length(xtick), by=2)
        even <- seq(2, length(xtick), by=2)
        for(i in odd) {
          axis(side=1, at=xtick[i], labels="", cex.axis = 1.5)
          axis(side=1, at=xtick[i], labels=xticklabel[i], line=-0.2, tick=FALSE, cex.axis = 1.5)
        }
        for(i in even) {
          axis(side=1, at=xtick[i], labels="", cex.axis = 1.5)
          axis(side=1, at=xtick[i], labels=xticklabel[i], line=+0.6, tick=FALSE, cex.axis = 1.5)
        }
      }
    }
