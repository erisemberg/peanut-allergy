...
png('figs/backcross-temps.png', width = 550)
bctemp
dev.off()
```
...
```{r hclusttree, include=F}
#png("figs/temp_clusters.png", width=1000)
...
png("figs/bc-temps-by-cluster.png", width=700)
pclust + big_theme
dev.off()
```
...
png("figs/pca-by-cluster.png", width = 700)
pcaplot + big_theme
dev.off()

pcaplot_pub <- fviz_pca_ind(pca, habillage = WB02$pheno$cluster, geom="point", 
                            title = "", addEllipses = TRUE, mean.point = FALSE, 
                            pointsize = 1, legend.title = 'Cluster') 
```
...
```{r clust2vs3}
WB02$pheno$cluster_2vs3 <- ifelse(WB02$pheno$cluster == 2, 0, 
                                  ifelse(WB02$pheno$cluster == 3, 1, NA))
```
...
```{r ssvstemp}
ssvstemp <- ggplot(data = WB02$pheno, mapping = aes(x = as.factor(Symptom_score), 
                                                    y = Temp_aac)) + 
...
png('figs/supplemental/temp-vs-symptomscore.png', width=600)
ssvstemp + big_theme
dev.off()
```
...
png('figs/supplemental/temp-vs-IgE.png', width=600)
igevstemp + big_theme
dev.off()
```
...
bfdat <- data.frame(aac = as.numeric(c(parent_aac, bc_aac)), 
                    group = c(rep('parent', length(parent_aac)), 
                              rep('bc', length(bc_aac))))
...
ggsave('figs/fig1/Figure1.png', fig1)
```

Version with subpanels showing clusters separately:

```{r fig1v2, warning=FALSE}
...
```{r models}
models <- read.csv("source_data/pnut_bc_models.csv", na.strings = c("", " "))
```
...
for (i in 1:length(normal_phenos)){
  ...
  formula <- paste0("pheno ~ ", ifelse(models$cov[i] == "", "", 
                                       paste0(models$cov[i], " + ")), 
                    "(1|batch) + (1|batch:cage)")
...
for (i in 1:nrow(peakstbl)){
  ...
  cov <- models$cov[models$obj == mod]
  if(is.na(cov)) { cov <- NULL }
  
  unadj.pval <- get_unadj_pval(WB02, pheno.col, marker, cov)
...
for (i in 1:nrow(peakstbl)){
  ...
  h2 <- get_var_expl(WB02, pheno.col, marker, cov)
...
png('figs/outcome-scree-plot.png', width = 600)
...
png('figs/supplemental/outcome-mtt-pca.png')
fviz_pca_var(outcome_pca.mtt, repel = TRUE)
dev.off()

png('figs/supplemental/outcome-mtt-screeplot.png')
fviz_screeplot(outcome_pca.mtt)
dev.off()
```

Try factor analysis. Clear elbow at 1, so try with 1 factor:
...
ensure_directory('figs/fig3')
png('figs/fig3/T15-Qpa1-Qpa2-boxplot.png', width=650)
pcolbym
dev.off()
```
...
png('figs/fig3/Taac-Qpa1-Qpa2-boxplot.png', width=650)
pcolbymaac
dev.off()
```
...
jpeg("figs/fig3/Taac-Qpa1Qpa2wsig.jpeg", width = 650)
...
png("figs/fig3/m5m10cluster.png", width=600)
