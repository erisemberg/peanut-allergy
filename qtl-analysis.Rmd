...
ggsave('figs/fig1/Figure1.jpeg', fig1, width = 700)
```
...
plot_scans(models = models[-which(models$obj == 'IgE'),], save = TRUE, 
           save.dir = 'figs/scans/', ylim = c(0, 4.5))
plot_scans(models = models[which(models$obj == 'IgE'),], save = TRUE, 
           save.dir = 'figs/scans/')
```
...
plot_modX(cluster_1vs23, alternate.chrid = TRUE, xlab = "", ylab = "", 
          main = "Temperature trajectory cluster", cex.main = 2, cex.axis = 2, 
          bandcol = "gray90", ylim = c(0, 4.5))
...
### Stacked bar plots for categorical phenotypes
...
xlab <- as.expression(bquote(italic(.('Qpa2'))*" (chr5) "*Genotype))
...
ggplot(data = ggdatsum, mapping = aes(x = Genotype, y = Frequency, fill = Cluster)) + 
  ...
  scale_fill_manual(values = c('#1B9E77', '#88CCEE')) + 
  labs(x = xlab) +
  big_pxg_theme
dev.off()
```
...
xlab <- as.expression(bquote(italic(.('Qpa4'))*" (chr6) "*Genotype))
...
ggplot(data = ggdatsum, mapping = aes(x = Genotype, y = Frequency, fill = Min_Temp_Time)) + 
  ...
  labs(x = xlab, fill = "Min temp time") + 
  big_pxg_theme
dev.off()
```
...
```{r qpa3ss}
ggdat2 <- data.frame(Genotype = pull.geno(WB02, chr=17)[,'S6Y171422147'], 
                     'Symptom score' = WB02$pheno$Symptom_score)
...
xlab <- as.expression(bquote(italic(.('Qpa3'))*" (chr17) "*Genotype))
...
ggplot(data = ggdat2sum, mapping = aes(x = Genotype, y = Frequency, fill = Symptom.score)) + 
  geom_bar(stat = 'identity', position = 'fill') +
  scale_fill_manual(values = blues) + 
  labs(x = xlab, fill = "Symptom score") +
  big_pxg_theme
dev.off()
```
...
*No, PNsIgE QTL do not improve model predicting temp response.*
...
png('figs/supplemental/scree-plot.png', width = 600)
...
png('figs/supplemental/screeplot-mtt.png')
...
ensure_directory('figs/multi-qtl')
png('figs/multi-qtl/T15-Qpa1-Qpa2-boxplot.png', width=650)
pcolbym
dev.off()
```
...
png('figs/multi-qtl/Taac-Qpa1-Qpa2-boxplot.png', width=650)
pcolbymaac
dev.off()
```
...
xlab <- as.expression(bquote(italic(.('Qpa1'))*Genotype))

jpeg("figs/multi-qtl/Qpa1Qpa2wsig.jpeg", width = 650)
ggplot(data = df, mapping = aes(x = geno, y = pheno, fill = geno2)) + 
     geom_boxplot() +
     scale_fill_manual(values = c('#994455', '#6699CC'), name = bquote(italic(Qpa2)~Genotype)) + 
     ...
     labs(y = "Temp AAC (normalized)", x = xlab) +
     poster_theme
dev.off()
```
...
png("figs/multi-qtl/m5m10cluster.png", width=600)
